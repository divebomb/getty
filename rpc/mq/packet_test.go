...
func TestPacket_GetAssignedQueueResponse(t *testing.T) {
	...
	ioBuffer := gxbytes.GetBytesBuffer()
	ioBuffer.Write(byteStreams)
	defer gxbytes.PutBytesBuffer(ioBuffer)

	var pkg TopicMetadataResponse
	_, err := pkg.Unmarshal(ioBuffer)
	if  err != nil {
		t.Errorf("pkg.Unmarshal() = error:%+v", err)
	}
	if !pkg.Validate() {
		t.Errorf("this is not a TopicMetadataResponse")
	}

	outBuffer, err := pkg.Marshal()
	if err != nil {
		t.Errorf("pkg.Marshal() = error:%+v", err)
	}
	defer gxbytes.PutBytesBuffer(outBuffer)
...
func TestGetConsumersHeader(t *testing.T) {
	...
	_, err := pkg.Unmarshal(ioBuffer)
	if err != nil {
		t.Errorf("GetConsumersRequest.Unmarshal() = error %s", err)
	}
	if !pkg.Validate() {
...
func TestPacket_ProducerHeartbeatHeader(t *testing.T) {
	...
	_, err := pkg.Unmarshal(ioBuffer)
	if err != nil {
		t.Errorf("HeartbeatRequest.Unmarshal() = error %s", err)
	}
	if !pkg.Validate() {
...
func TestPacket_GetHeartbeatResponse(t *testing.T) {
	...
	_, err := pkg.Unmarshal(ioBuffer)
	if err != nil {
		t.Errorf("pkg.Unmarshal() = error %s", err)
	}
