...
func TestTopicMetadataRequest(t *testing.T) {
	// test validate
	pkt := NewRequest(CLIENT_HEARTBEAT, nil)
	var req TopicMetadataRequest
	req.Packet = *pkt
	assert.False(t, req.Validate())

	pkt = NewRequest(GET_TOPIC_METADATA, nil)
	req.Packet = *pkt
...
func TestTopicMetadataResponse_SetMetadata(t *testing.T) {
	var rsp TopicMetadataResponse
	err := rsp.SetMetadata(nil)
	assert.Nil(t, err)

	meta, err := rsp.GetMetadata()
	assert.NotNil(t, meta)
}

func TestGetAssignedQueuesRequest(t *testing.T) {
	// test validate
	pkt := NewRequest(CLIENT_HEARTBEAT, nil)
	var req GetAssignedQueuesRequest
	req.Packet = *pkt
	assert.False(t, req.Validate())

	pkt = NewRequest(GET_ASSIGNED_QUEUES, nil)
	req.Packet = *pkt
	assert.True(t, req.Validate())
...
func TestGetAssignedQueuesResponse(t *testing.T) {
	// test validate
	pkt := NewResponse(TIMEOUT, 0x1234)
	var rsp GetAssignedQueuesResponse
	rsp.Packet = *pkt
	assert.False(t, rsp.Validate())

	pkt = NewResponse(SUCCESS, 0x1234)
	rsp.Packet = *pkt
	assert.True(t, rsp.Validate())
	...
	// test meta
	_, err = rsp.GetMetadata()
	assert.NotNil(t, err)

	var list MessageQueueList
	rsp.SetMetadata(list)
}

func TestConsumerListChangeRequest(t *testing.T) {
	// test validate
	pkt := NewRequest(CLIENT_HEARTBEAT, nil)
	var req ConsumerListChangeRequest
	req.Packet = *pkt
	assert.False(t, req.Validate())

	pkt = NewRequest(CONSUMER_LIST_CHANGE, nil)
	req.Packet = *pkt
	assert.True(t, req.Validate())
...
func TestSendMessageRequest(t *testing.T) {
	...
	pkt = NewRequest(SEND_MESSAGE, nil)
	req.Packet = *pkt
	assert.True(t, req.Validate())
	...
	// test Rebuild
	req.header = &WriteMessageHeader{}
	err = req.Rebuild(12345)
	assert.Nil(t, err)
}

func TestSendTxSubmitMessageRequest(t *testing.T) {
	// test validate
	pkt := NewRequest(CLIENT_HEARTBEAT, nil)
	var req SendTxSubmitMessageRequest
	req.Packet = *pkt
	assert.False(t, req.Validate())

	pkt = NewRequest(SUBMIT_TRANSACTIONAL_MESSAGE, nil)
	req.Packet = *pkt
	assert.True(t, req.Validate())
...
func TestTxCheckMessageRequest(t *testing.T) {
	// test validate
	pkt := NewRequest(CLIENT_HEARTBEAT, nil)
	var req TxCheckMessageRequest
	req.Packet = *pkt
	assert.False(t, req.Validate())

	pkt = NewRequest(CHECK_TRANSACTIONAL_MESSAGE, nil)
	req.Packet = *pkt
	assert.True(t, req.Validate())
...
func TestSendBatchMessageRequest(t *testing.T) {
	// test validate
	pkt := NewRequest(CLIENT_HEARTBEAT, nil)
	var req SendBatchMessageRequest
	req.Packet = *pkt
	assert.False(t, req.Validate())

	pkt = NewRequest(SEND_BATCH_MESSAGE, nil)
	req.Packet = *pkt
	assert.True(t, req.Validate())
	...
	_, err := req.GetHeader()
	assert.NotNil(t, err)
	req.header = &WriteBatchMessageHeaderEx{}
	...
	// test Rebuild
	err = req.Rebuild(0x1234)
	assert.Nil(t, err)
}

func TestFetchMessageRequest(t *testing.T) {
	// test validate
	pkt := NewRequest(CLIENT_HEARTBEAT, nil)
	...
	pkt = NewRequest(FETCH_MESSAGE, nil)
	req.Packet = *pkt
	assert.True(t, req.Validate())
	...
	_, err := req.GetHeader()
	assert.NotNil(t, err)
	req.header = &ReadMessageHeader{}

	// test Rebuild
	err = req.Rebuild(&ReadMessageHeader{})
	assert.Nil(t, err)
}

func TestResetOffsetHeader(t *testing.T) {
	// test validate
	pkt := NewRequest(CLIENT_HEARTBEAT, nil)
	...
	pkt = NewRequest(RESET_OFFSET, nil)
	req.Packet = *pkt
	assert.True(t, req.Validate())
...
func TestScheduleMessageRequest(t *testing.T) {
	// test validate
	pkt := NewRequest(CLIENT_HEARTBEAT, nil)
	...
	pkt = NewRequest(SCHEDULE_MESSAGE, nil)
	req.Packet = *pkt
	assert.True(t, req.Validate())
...
func TestScheduleMessageResponse(t *testing.T) {
	// test validate
	pkt := NewResponse(TIMEOUT, 0x12345)
	var rsp ScheduleMessageResponse
	rsp.Packet = *pkt
	assert.False(t, rsp.Validate())
...
func TestUnregisterClientRequest(t *testing.T) {
	// test validate
	pkt := NewRequest(CLIENT_HEARTBEAT, nil)
	var req UnregisterClientRequest
	req.Packet = *pkt
	assert.False(t, req.Validate())

	pkt = NewRequest(CLIENT_UNREGISTER, nil)
	req.Packet = *pkt
	assert.True(t, req.Validate())
	...
	_, err := req.GetHeader()
	assert.NotNil(t, err)
	req.header = &ClientLogoutHeader{}

	// test Rebuild
	err = req.Rebuild(&ClientLogoutHeader{})
	assert.Nil(t, err)
}

func TestUnregisterClientResponse(t *testing.T) {
	// test validate
	pkt := NewResponse(TIMEOUT, 0x12345)
	var rsp UnregisterClientResponse
	rsp.Packet = *pkt
	assert.False(t, rsp.Validate())

	pkt = NewResponse(SUCCESS, 0x12345)
	rsp.Packet = *pkt
	assert.True(t, rsp.Validate())
}
...
func TestFetchMessageRequest_Rebuild(t *testing.T) {
	...
	rs2 := rs.Clone()
	rs2.SetCommandID(30)
	rs2.OutputHeaderHex(true)
	rs2.OutputBodyHex(true)
}

func TestPacket(t *testing.T) {
	rq := NewRequest(GET_OFFSET, []byte{0x1})
	rq.setHeader([]byte{0x1})
	rq.setBody([]byte{0x1})

	rq2 := rq.Clone()
	rq2.SetCommandID(30)
	rq2.OutputHeaderHex(true)
	rq2.OutputBodyHex(true)

	t.Logf("type:%v, code:%v, id:%v", rq2.CommandType(), rq2.CommandCode(), rq2.CommandID())
	rq2.MarkOneWay()
	rq2.MarkRequest()
	t.Logf("isOneWay:%v, rqp:%v, req:%v, s:%s", rq2.IsOneWay(), rq2.IsResponse(), rq2.IsRequest(), rq2.String())
}

func TestPacket_Unmarshal(t *testing.T) {
	// test total length
	byteStreams := []byte{0x1}
	ioBuffer := gxbytes.GetBytesBuffer()
	ioBuffer.Write(byteStreams)
	defer gxbytes.PutBytesBuffer(ioBuffer)
	...
	var pkg TopicMetadataResponse
	_, err := pkg.Unmarshal(ioBuffer)
	assert.True(t, err == ErrNotEnoughStream)

	// test packet length
	byteStreams = []byte{0x0F, 0x0, 0x0, 0x0F, 0x0}
	ioBuffer1 := gxbytes.GetBytesBuffer()
	ioBuffer1.Write(byteStreams)
	defer gxbytes.PutBytesBuffer(ioBuffer1)

	var pkg1 TopicMetadataResponse
	_, err = pkg1.Unmarshal(ioBuffer1)
	assert.True(t, err == ErrNotEnoughStream)

	// test magic
	byteStreams = []byte{0xEF, 0x0, 0x0, 0x0F, 0x0, 0x2, 0x3, 0x4, 0x5, 0x6}

	ioBuffer2 := gxbytes.GetBytesBuffer()
	ioBuffer2.Write(byteStreams)
	defer gxbytes.PutBytesBuffer(ioBuffer2)

	var pkg2 TopicMetadataResponse
	_, err = pkg2.Unmarshal(ioBuffer2)
	assert.NotNil(t, err)

	var magicArray [4]byte
	var magic int32
	magic = MAGIC_CODE
	binary.BigEndian.PutUint32(magicArray[0:], uint32(magic))
	byteStreams[4] = magicArray[0]
	byteStreams[5] = magicArray[1]
	byteStreams[6] = magicArray[2]
	byteStreams[7] = magicArray[3]

	// check version
	ioBuffer3 := gxbytes.GetBytesBuffer()
	ioBuffer3.Write(byteStreams)
	defer gxbytes.PutBytesBuffer(ioBuffer3)

	_, err = pkg2.Unmarshal(ioBuffer3)
	assert.NotNil(t, err)
}
